import {
  ...
  ɵɵngDeclareInjector,
  ɵɵngDeclareNgModule
} from "./chunk-CNHOTKRN.js";

// angular:jit:template:src/app/app.html
var app_default;
var init_app = __esm({
  "angular:jit:template:src/app/app.html"() {
    app_default = `<div class="w-11/12 sm:w-10/12 lg:w-8/12 flex flex-col justify-center items-center">
  
  <div class="relative flex-1 flex flex-col w-[100%] p-5 border-2 border-black rounded-2xl overflow-hidden shadow-[5px_6px_0px_rgba(0,0,0,0.7)]">
    
    <!-- Background divs  -->
    <div class="absolute inset-0 bg-[radial-gradient(#00000080_1px,transparent_0)] bg-[size:30px_30px] -z-10" [ngClass]="inputTypeService.activeInputType() === 'text' ? 'opacity-100' : 'opacity-0'" style="transition: opacity 0.5s ease-in-out;"></div>
    <div class="absolute inset-0 bg-[radial-gradient(circle,transparent_20%,white_100%)] -z-10" [ngClass]="inputTypeService.activeInputType() === 'text' ? 'opacity-100' : 'opacity-0'" style="transition: opacity 0.5s ease-in-out;"></div>
    <div class="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(13,17,23,0.9)_10%,rgba(4,6,9,0.9)_40%,rgba(0,0,0,0.9)_100%)] -z-10" [ngClass]="inputTypeService.activeInputType() === 'file' ? 'opacity-100' : 'opacity-0'" style="transition: opacity 0.5s ease-in-out;"></div>

    <!-- navbar -->
    <div class="flex gap-2 items-center">
      <!-- navbar - slide button  -->
      <div class="border-2 [box-shadow:inset_5px_5px_0px_rgba(0,0,0,0.7)] w-max rounded-lg">
        <div class="h-10 w-10 "></div>
      </div>
      <div class=" rounded-lg flex items-center">
        <div class="flex items-center gap-2">
          <div class="h-4 w-4 rounded-full flex justify-between items-center" [ngClass]="status() === 'live' ? 'bg-green-500' : 'bg-yellow-500'">
            <div class="h-4 w-4 rounded-full animate-ping" [ngClass]="status() === 'live' ? 'bg-green-500' : 'bg-yellow-500'"></div>
          </div>
          <p class="text-xs text-black/[0.2]">{{ status() === 'live' ? ' ' : 'Waking up, usually takes a few minutes'}}</p>
        </div>
      </div>
    </div>
    
    <div class="flex-1 flex">
      <div class="min-h-60 lg:min-h-80 flex-1">
...
// src/app/input-type.service.ts
var InputTypeService;
var init_input_type_service = __esm({
  "src/app/input-type.service.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    InputTypeService = class InputTypeService2 {
      activeInputType = signal("text");
    };
...
// src/app/api-status.service.ts
var ApiStatusService;
var init_api_status_service = __esm({
  "src/app/api-status.service.ts"() {
    "use strict";
    init_tslib_es6();
    init_core();
    ApiStatusService = class ApiStatusService2 {
      isApiLive = signal(false);
      constructor() {
      }
      static ctorParameters = () => [];
    };
    ApiStatusService = __decorate([
      Injectable({
        providedIn: "root"
      })
    ], ApiStatusService);
...
var init_app3 = __esm({
  "src/app/app.ts"() {
    ...
    init_operators();
    init_esm();
    init_api_status_service();
    App = class App2 {
      http;
      apiStatusService;
      inputTypeService;
      title = signal("Sentiment_Analyzer");
      status = signal("waking up");
      statusSubscription;
      constructor(http, apiStatusService, inputTypeService) {
        this.http = http;
        this.apiStatusService = apiStatusService;
        this.inputTypeService = inputTypeService;
      }
      ngOnInit() {
        this.statusSubscription = interval(5e3).pipe(switchMap(() => this.http.get("/api", { observe: "response", responseType: "text" }).pipe(catchError(() => of(null))))).subscribe((response) => {
  ...
  "src/app/app.spec.ts"(exports) {
    ...
    describe("App", () => {
      ...
      it("should render title", () => {
        ...
        expect(compiled.querySelector("h1")?.textContent).toContain("Hello, Sentiment_Analyzer");
      });
    });
  }
});
